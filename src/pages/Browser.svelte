<script>
  ...
  import { push, link, routeParams } from "svelte-spa-history-router";
  ...
  $: onKeyChange($routeParams.key);

  async function loadFile() {
    try {
      const fh = await project.getContent($routeParams.key);
...
<GrantAccess {project}>
  <main id="Home">
    ...
    >
      ...
      <div class="viewer">
        <Viewer
          url={ project.previewUrl($routeParams.key) }
          bind:needsReload
          bind:needsResize
